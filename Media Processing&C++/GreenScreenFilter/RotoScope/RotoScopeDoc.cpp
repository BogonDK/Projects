...
BEGIN_MESSAGE_MAP(CRotoScopeDoc, CDocument)
	...
	ON_COMMAND(ID_MOUSEMODE_GOOMBA, &CRotoScopeDoc::OnMousemodeGoomba)
	ON_COMMAND(ID_BIGGOOMBA, &CRotoScopeDoc::OnBigGoomba)
END_MESSAGE_MAP()
...
CRotoScopeDoc::CRotoScopeDoc()
{
	...
	m_backgroundMode = 0;
	m_goomba.LoadFile(L"goomba.png");
	//OnEditSetvariables();
}
	...
	else if (m_backgroundMode == 1)
	{
		for (int r = 0; r < m_image.GetHeight() && r < m_initial.GetHeight(); r++)
		{
			for (int c = 0; c < m_image.GetWidth() && c < m_initial.GetWidth(); c++)
			{
				...
				if (colorG > 0 && colorR < 130 && colorB < 130)
				{
					m_image[r][c * 3] = 0;
					m_image[r][c * 3 + 1] = 0;
					m_image[r][c * 3 + 2] = 0;
...
void CRotoScopeDoc::DrawGoomba(CGrImage &image, int x1, int y1)
{
	//allow undo of placing
	m_images.push(m_image);
	if (m_goombaAngle == 0) {
		for (int r = 0; r < m_goomba.GetHeight(); r++)
		{
			for (int c = 0; c < m_goomba.GetWidth(); c++)
			{
				//make sure point is inside image
				if (r + y1 < m_image.GetHeight() && c + x1 < m_image.GetWidth())
				{

					if (r * m_goombaScale < m_goomba.GetHeight() && c * m_goombaScale < m_goomba.GetWidth())
					{
						if (m_goomba[r * m_goombaScale][c * m_goombaScale * 4 + 2] >= 50)
						{
							m_image[r + y1][(c + x1) * 3] = m_goomba[r * m_goombaScale][c * m_goombaScale * 4];
							m_image[r + y1][(c + x1) * 3 + 1] = m_goomba[r * m_goombaScale][c * m_goombaScale * 4 + 1];
							m_image[r + y1][(c + x1) * 3 + 2] = m_goomba[r * m_goombaScale][c * m_goombaScale * 4 + 2];
						}
		...
		CGrImage rotatedImage = RotateObject(m_goomba, m_goombaAngle);
		for (int r = 0; r < rotatedImage.GetHeight(); r++)
		{
			for (int c = 0; c < rotatedImage.GetWidth(); c++)
			{
				...
				if (r + y1 < m_image.GetHeight() && c + x1 < m_image.GetWidth())
				{
					...
					if (r * m_goombaScale < rotatedImage.GetHeight() && c * m_goombaScale < rotatedImage.GetWidth())
					{
						if (rotatedImage[r * m_goombaScale][c * m_goombaScale * 4 + 2] >= 50)
						{
							m_image[r + y1][(c + x1) * 3] = rotatedImage[r * m_goombaScale][c * m_goombaScale * 4];
							m_image[r + y1][(c + x1) * 3 + 1] = rotatedImage[r * m_goombaScale][c * m_goombaScale * 4 + 1];
							m_image[r + y1][(c + x1) * 3 + 2] = rotatedImage[r * m_goombaScale][c * m_goombaScale * 4 + 2];
						}
...
void CRotoScopeDoc::OnMousemodeGoomba()
{
	m_mode = 4;
}


void CRotoScopeDoc::OnBigGoomba()
{
	if (m_goombaScale > 2)
	{ m_goombaScale = 2;}
	else {m_goombaScale = 10;}

	if (m_goombaAngle == 0)
	{
		m_goombaAngle = 180;
	}
	else { m_goombaAngle = 0; }
}
